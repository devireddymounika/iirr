...
function setCookie(name, value, days) {
	...
	// respectively
	var expstring = (typeof days != "undefined") ? expireDate
			.setDate(expireDate.getDate() + parseInt(days)) : expireDate
			.setDate(expireDate.getDate() - 5)
...
if (manual_or_random == "manual") { // IF MANUAL MODE
	...
	if (document.getElementById && selectedtitle != null) // load user chosen
															// style sheet from
															// cookie if there
															// is one stored
	...
	if (randomsetting == "eachtime")
		...
		if (getCookie("mysheet_s") == null) // if "mysheet_s" session cookie is
											// empty
			document.cookie = "mysheet_s=" + setStylesheet("", "random")
					+ "; path=/" // activate random alt stylesheet while
									// remembering its "title" value
		else
			setStylesheet(getCookie("mysheet_s")) // just activate random alt
													// stylesheet stored in
													// cookie
	} else if (randomsetting.search(/^[1-9]+ days/i) != -1) { // if "x days"
																// setting
		if (getCookie("mysheet_r") == null
				|| parseInt(getCookie("mysheet_r_days")) != parseInt(randomsetting)) { 
		
			setCookie("mysheet_r", setStylesheet("", "random"),
					parseInt(randomsetting)) // activate random alt
												// stylesheet while remembering
												// its "title" value
...
$(document).ready(function() {
	$('.dark').click(function(){	
		...
		document.cookie="username=Swapna Yanala";
		$(".light").show();
		$(".dark").hide();
		$(".high-contrast.light").focus();		
		
		chooseStyle('change', 60);
	});
